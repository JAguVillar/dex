...
async function getAllPokemonsWithGenerations() {
  try {
    ...
    // Step 2: Fetch generation for each Pokémon
    const pokemonsWithGenerations = await Promise.all(
      pokemons.map(async (pokemon, index) => {
        ...
        return {
          ...
          sprite: `${BASE_SPRITE_URL}${index + 1}.png`,
          sprite_shiny: `${BASE_SPRITE_SHINY_URL}${index + 1}.png`,
          arttwork: `${BASE_ARTWORK_URL}${index + 1}.png`,
          artwork_shiny: `${BASE_ARTWORK_SHINY_URL}${index + 1}.png`,
        };
      })
    );

    return pokemonsWithGenerations;
  } catch (error) {
    console.error(error);
  }
}
...
function getGeneration(generation) {
  const generationNumber = generation.split("-");
  switch (generationNumber[1]) {
    case "i":
      return 1;
    case "ii":
      return 2;
...
// Example usage
getAllPokemonsWithGenerations().then((pokemons) => {
  if (pokemons) {
